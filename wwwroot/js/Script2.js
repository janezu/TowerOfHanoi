...
function postResult(nMoves, uId, cId, eTime) {
    ...
    $.ajax({
        url: "Home/Create/",
        type: 'POST',
        data: parm, dataType: "json", success: function () {}
    })
...
function create() {
    ...
    uStolp(end - 1); cDisc(start); resetTimer(); startTimer(); getInfo(), getScores(); setVariation();
};
    ...
    if (val > 1) {
        ...
        for (i = dr; i >= 0; i--) {
            ...
            if (dar[i] != null && dar[i].firstElementChild.getAttribute("data-nr") == val) {
                ...
                var kah = "#" + dar[i].id; console.log(dar[i]); var cln = dar[i].cloneNode(true);; $(kah).remove(); document.getElementById("garbo").insertBefore(cln, document.getElementById("garbo").children[0]);
            }}
        ...
        for (i = nr; i >= 0; i--) {
          ...
          /*  if (parnt[i].firstElementChild.classList.contains("d-none") == false) {
            ...
            if (parnt[i] != null && parnt[i].firstElementChild.getAttribute("data-nr") != val) {
                ...
                var kah = "#" + parnt[i].id; console.log(parnt[i]); var cln = parnt[i].cloneNode(true);; $(kah).remove(); document.getElementById("odlagalisce").insertBefore(cln, document.getElementById("odlagalisce").children[0]);
            }
...
function pVariations(dis) {
    ...
    for (i = dr; i >= 0; i--) {
        if (dar[i] != null && dar[i].firstElementChild.getAttribute("data-var") == varid) {
            if (dar[i].classList.contains("active")) { dar[i].classList.remove("active") };
            var kah = "#" + dar[i].id; console.log(dar[i]); var cln = dar[i].cloneNode(true);; $(kah).remove(); document.getElementById("garbon").insertBefore(cln, document.getElementById("garbon").children[0]);
        }
    }
    ...
    for (i = nr; i >= 0; i--) {
        ...
        if (parnt[i] != null && parnt[i].firstElementChild.getAttribute("data-var") != varid) {
            ...
            if (parnt[i].classList.contains("active")) { parnt[i].classList.remove("active") };
            var kah = "#" + parnt[i].id; console.log(parnt[i]); var cln = parnt[i].cloneNode(true);; $(kah).remove(); document.getElementById("odlagaliscec").insertBefore(cln, document.getElementById("odlagaliscec").children[0]);
        }
...
function uStolp(test) {
    ...
    if (stStolpov > 0 && stDiskov > 0) {
        ...
        for (i = 0; i < stStolpov; ++i) {
            ...
            htmlstolp.setAttribute("index", i + 1);
            ...
            htmlcrta.setAttribute("id", "tretja");
            htmlcrta.setAttribute("discs", "0");
