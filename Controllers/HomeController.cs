...
namespace TowerOfHanoi.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        [AllowAnonymous]
        public async Task<IActionResult> Scores(int? Var, string searchString, int? Con, int? diskFilter)
        {
            ...
            if (Var != null && Var != 0)
            {
                scores = scores.Where(x => x.Optimal.Configuration.Variation.VariationID == Var);
            }
            ...
            if (Con != null && Con != 0)
            {
                scores = scores.Where(x => x.Optimal.Configuration.ConfigurationID == Con);
            }
            if (Con != null)
            {
                if (Con > 0)
